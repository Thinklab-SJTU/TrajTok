...
def sample_next_token_traj(
    ...
    if (
        ...
        if sampling_scheme.criterium == "topk_prob_sampled_with_dist":
            ...
            gt_contour = cal_polygon_contour(
                pos_next_gt, head_next_gt, token_agent_shape.to(pos_next_gt.device)
            )
    ...
    if sampling_scheme.criterium == "threshold":
        next_token_prob = next_token_logits.softmax(-1)
        threshold = sampling_scheme.threshold
        topk_probs, topk_indices = torch.topk(
            next_token_prob, sampling_scheme.num_k, dim=-1, sorted=False
        )

        mask = topk_probs >= threshold
        mask[mask.sum(dim=-1) == 0] = True
        topk_probs_above_threshold = topk_probs.masked_fill(~mask, 0.0)
        samples = Categorical(probs=topk_probs_above_threshold).sample() 
...
def sample_next_token_traj_and_heading(
    token_traj: Tensor,  # [n_agent, n_token, 4, 2]
    token_traj_all: Tensor,  # [n_agent, n_token, 6, 4, 2]
    token_heading: Tensor,
    sampling_scheme: DictConfig,
    # ! for most-likely sampling
    next_token_logits: Tensor,  # [n_agent, n_token], with grad
    # ! for nearest-pos sampling, sampling near to GT
    next_heading_logits: Tensor
    ...
    assert sampling_scheme.criterium == "topk_prob"
    ...
    next_heading_logits = next_heading_logits.detach()
    topk_logits_heading, topk_indices_heading = torch.topk(
        next_heading_logits, 3, dim=-1, sorted=False)
    topk_logits_heading = topk_logits_heading / sampling_scheme.temp
    samples_heading = Categorical(logits=topk_logits_heading).sample()  # [n_agent] in K
    next_heading_idx = topk_indices_heading[range_a, samples_heading]
    
    next_heading =  token_heading[next_heading_idx]
    next_token_traj_all = token_traj_all[range_a, next_token_idx]


    return next_token_idx, next_token_traj_all, next_heading_idx, next_heading
