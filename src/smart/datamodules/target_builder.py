...
class WaymoTargetBuilderTrain(BaseTransform):
    def __init__(self, max_num: int, separate: bool = False) -> None:
    ...
    def __call__(self, data) -> HeteroData:
        ...
        if train_mask.sum() > self.max_num:  # too many vehicle
            ...
            data["agent"]["train_mask_role"] = role_train_mask
            data["agent"]["train_mask_extra"] = torch.zeros_like(data["agent"]["train_mask"]).bool()
            data["agent"]["train_mask_extra"][selected_indices] = True
            
            
        else:
            data["agent"]["train_mask"] = train_mask  # [n_agent]
            data["agent"]["train_mask_role"] = role_train_mask
            data["agent"]["train_mask_extra"] = extra_train_mask
