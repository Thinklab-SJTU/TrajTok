...
class SMARTAgentDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.token_emb_veh = FourierEmbedding(
            input_dim=input_dim_token, hidden_dim=hidden_dim, num_freq_bands=num_freq_bands
        )
        self.token_emb_ped = FourierEmbedding(
            input_dim=input_dim_token, hidden_dim=hidden_dim, num_freq_bands=num_freq_bands
        )
        self.token_emb_cyc = FourierEmbedding(
            input_dim=input_dim_token, hidden_dim=hidden_dim, num_freq_bands=num_freq_bands
        )
        ...
        self.fusion_emb = MLPEmbedding(input_dim=self.hidden_dim * fusion_num, hidden_dim=self.hidden_dim
        )
        ...
        if self.multi_token_size:
            ...
            self.heading_predict_head = nn.ModuleDict()
            for key, value in n_token_agent.items():
                self.token_predict_head[key] = MLPLayer(
                    input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=value
                )
        else:
    ...
    def forward(
        ...
        pos_a = tokenized_agent["tokenized_pos"]
        head_a = tokenized_agent["tokenized_heading"]
        type_mask = tokenized_agent["type_mask"]
        role_mask = tokenized_agent["role_mask"]
        ...
        feat_a = self.agent_token_embedding(
            agent_token_index=tokenized_agent["token_idx"],  # [n_ag, n_step]
        ...
        if self.multi_token_size:
            next_token_logits = {}
            for agent_type, t_mask in type_mask.items():
                next_token_logits[agent_type] = self.token_predict_head[agent_type](feat_a[t_mask])[:, 1:-1]
        else:
            next_token_logits = self.token_predict_head(feat_a)[:, 1:-1]

        return {
            # action that goes from [(10->15), ..., (85->90)]
            "next_token_logits": next_token_logits,  # [n_agent, 16, n_token]
            "next_token_valid": tokenized_agent["valid_mask"][:, 1:-1],  # [n_agent, 16]
            # for step {5, 10, ..., 90} and act [(0->5), (5->10), ..., (85->90)]
            "gt_idx": tokenized_agent["token_idx"][:, 2:],  # [n_agent, 16]
            "gt_valid_mask": tokenized_agent["valid_mask"][:, 2:],  # [n_agent, 16]
            "type_mask": type_mask,  # [n_agent]
        }
    ...
    def inference(
        ...
        pos_a = tokenized_agent["tokenized_pos"][:, :step_current_2hz].clone()
        head_a = tokenized_agent["tokenized_heading"][:, :step_current_2hz].clone()
        head_vector_a = torch.stack([head_a.cos(), head_a.sin()], dim=-1)
        pred_idx = tokenized_agent["token_idx"].clone()
        ...
        (
            ...
            agent_token_index=tokenized_agent["token_idx"][:, :step_current_2hz],
            trajectory_token_veh=tokenized_agent["trajectory_token_veh"],
            trajectory_token_ped=tokenized_agent["trajectory_token_ped"],
            trajectory_token_cyc=tokenized_agent["trajectory_token_cyc"],
            pos_a=pos_a,
        ...
        next_token_logits_list = {agent_type: [] for agent_type in type_mask.keys()}
        ...
        for t in range(n_step_future_2hz):  # 0 -> 15
            ...
            # ! get outputs
            if self.multi_token_size:
                next_token_logits_pad = torch.zeros(
                    [n_agent, max(self.n_token_agent.values())],
                    dtype=feat_a.dtype,
                    device=feat_a.device,
                )
                next_token_idx_by_type = {}
                next_token_prob_by_type = {}
                next_token_traj_all_by_type = {}
                
                for agent_type, t_mask in type_mask.items():
                    if  t_mask.sum() > 0:
                        t_mask_cpu = t_mask.cpu()
                        next_token_logits_by_type = self.token_predict_head[agent_type](feat_a_now[t_mask])
                        next_token_logits_list[agent_type].append(next_token_logits_by_type)
                        next_token_idx_by_type[agent_type], next_token_traj_all_by_type[agent_type] = sample_next_token_traj(
                        token_traj=tokenized_agent["token_traj"][agent_type],
                        token_traj_all=tokenized_agent["token_traj_all"][agent_type],
                        sampling_scheme=sampling_scheme,  # ! for most-likely sampling
                        next_token_logits=next_token_logits_by_type,  # ! for nearest-pos sampling
                        pos_now=pos_a[t_mask_cpu, t_now],  # [n_agent, 2]
                        head_now=head_a[t_mask_cpu, t_now],  # [n_agent]
                        pos_next_gt=tokenized_agent["gt_pos_raw"][t_mask_cpu, n_step],  # [n_agent, 2]
                        head_next_gt=tokenized_agent["gt_head_raw"][t_mask_cpu, n_step],  # [n_agent]
                        valid_next_gt=tokenized_agent["gt_valid_raw"][t_mask_cpu, n_step],  # [n_agent]
                        token_agent_shape=tokenized_agent["token_agent_shape"][t_mask_cpu],  # [n_token, 2]
                        ) 
                        next_token_prob_by_type[agent_type] = torch.softmax(next_token_logits_by_type, dim=-1)[torch.arange(t_mask.sum()), next_token_idx_by_type[agent_type]]
                ...
                next_token_probs = merge_by_type(next_token_prob_by_type, type_mask)
                ...
                next_token_idx, next_token_traj_all = sample_next_token_traj(
                    ...
                    sampling_scheme=sampling_scheme,  # ! for most-likely sampling
                    next_token_logits=next_token_logits, # ! for nearest-pos sampling
            ...
            diff_xy = next_token_traj_all[:, :, 0] - next_token_traj_all[:, :, 3]
            next_token_traj_all_xy =  next_token_traj_all.mean(dim=-2)[:,1:]
            next_token_traj_all_head = torch.arctan2(diff_xy[..., 1], diff_xy[..., 0])[:,1:]
            next_token_action_list.append(torch.cat([next_token_traj_all_xy[:, -1], next_token_traj_all_head[:,[-1]]], dim=-1))

            token_traj_global_xy, token_traj_global_head = transform_to_global(
                pos_local=next_token_traj_all_xy, 
                head_local=next_token_traj_all_head,
                pos_now=pos_a[:, t_now],  # [n_agent, 2]
                head_now=head_a[:, t_now],  # [n_agent]
            )

            if not self.training:
                pred_traj_10hz[:, t * 5 : (t + 1) * 5] = token_traj_global_xy
                pred_head_10hz[:, t * 5 : (t + 1) * 5] = token_traj_global_head

            # ! get pos_a_next and head_a_next, spawn unseen agents
            pos_a_next = token_traj_global_xy[:,-1]
            head_a_next = token_traj_global_head[:,-1]
        ...
        for key in next_token_logits_list.keys():
            if len(next_token_logits_list[key]) >0:
                next_token_logits_list[key] = torch.stack(next_token_logits_list[key], dim=1)

        return {
            # action that goes from [(10->15), ..., (85->90)]
            "next_token_logits": next_token_logits_list,  # [n_agent, 16, n_token]
            "next_token_valid": tokenized_agent["valid_mask"][:, 1:-1],  # [n_agent, 16]
            # for step {5, 10, ..., 90} and act [(0->5), (5->10), ..., (85->90)]
            "gt_idx": tokenized_agent["token_idx"][:, 2:],  # [n_agent, 16]
            "gt_valid_mask": tokenized_agent["valid_mask"][:, 2:],  # [n_agent, 16]
